...
<body>
  ...
  if ($daysSinceAppointment > 0) {
    ...
    } elseif ($daysSinceAppointment >= 30 && $daysSinceAppointment < 365) {
      $months = floor($daysSinceAppointment / 30);
      echo $months . ' ' . ($months === 1 ? 'month' : 'months');
    } else {
      $years = floor($daysSinceAppointment / 365);
      $remainingMonths = floor(($daysSinceAppointment % 365) / 30);
      if ($remainingMonths === 0) {
        echo $years . ' ' . ($years === 1 ? 'year' : 'years');
      } else {
        echo $years . ' ' . ($years === 1 ? 'year' : 'years') . ' ' . $remainingMonths . ' ' . ($remainingMonths === 1 ? 'month' : 'months');
      }
    }
  }
  ?>
